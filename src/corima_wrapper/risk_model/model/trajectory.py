...
class Trajectory:
    ...
    def future_prediction(
        ...
        if velocity.speed == 0:
            ...
            # replace this waypoint by the current position            
            if trajectory_id == "14":
                waypoints = waypoints_1
            if trajectory_id == "7":
                waypoints = waypoints_2_left
            if trajectory_id == "0":
                waypoints = waypoints_3
            idx = closest_pt_idx(position.x, position.y, waypoints)

            line = LineString(waypoints[idx:idx+50])
            pts = []
            for t in range(240):
